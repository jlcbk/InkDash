...
class VibeStatusTests(unittest.TestCase):
    ...
    def test_request_line_redaction_tolerates_malformed_urls(self):
        request_line = "GET http://[::1?token=secret&event=ok HTTP/1.1"
    ...
    def test_load_vibe_status_falls_back_to_legacy_when_new_file_is_not_object(self):
        legacy = Path(self.tmpdir.name) / "vibe_status.json"
        new_file = Path(self.tmpdir.name) / "inkdash_status.json"
        new_file.write_text("[]", encoding="utf-8")
        legacy.write_text(json.dumps({"state": "legacy-object"}), encoding="utf-8")
        ...
        self.assertEqual(status.get("state"), "legacy-object")
