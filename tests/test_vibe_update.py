...
class VibeUpdateTests(unittest.TestCase):
    ...
    def test_default_url_prefers_status_endpoint(self):
        with patch.dict(vibe_update.os.environ, {}, clear=True):
            args = vibe_update.parse_args([])

        self.assertEqual(args.url, "http://localhost:8080/api/status")
    ...
    def test_derive_health_url_from_vibe_url(self):
        ...
        self.assertEqual(
            vibe_update.derive_health_url("http://localhost:8080/api/status"),
            "http://localhost:8080/api/health",
        )
        self.assertEqual(
