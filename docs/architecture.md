...
InkDash（墨板）当前保持单进程、标准库优先的结构，目标是让旧 Kindle 浏览器、终端脚本和协作 agent 都能稳定读取同一份 Codex 用量和可选协作状态。

## 主要模块

| 文件 | 职责 |
| --- | --- |
| `app.py` | HTTP 服务、页面渲染、设置页、状态文件读写、Codex 用量读取、preset API。 |
| `status_model.py` | 纯状态模型逻辑：默认状态、字段规范化、事件截断、心跳过期判断和 patch 合并。 |
| `vibe_update.py` | 命令行状态读写工具，封装 `/api/status`、`/api/health`、preset 和心跳操作。 |
| `config.json` | 服务端口、刷新间隔、Codex 来源、状态过期阈值、可选写入 token。 |
| `inkdash_status.json` | 本地运行时状态文件，不提交到 Git；历史 `vibe_status.json` 可作为兼容回退读取。 |
...
脚本 / agent / Makefile
        |
        v
vibe_update.py  ----POST /api/status---->  app.py
        |                                |
        |                                v
        |                          inkdash_status.json
        |                                |
...
核心状态只存一份：`inkdash_status.json`。页面、纯文本端点、健康检查和 CLI 读取到的都是同一份状态；旧部署留下的 `vibe_status.json` 只作为迁移期回退读取。
...
| JSON API | `/api/status`、`/api/vibe` 兼容别名、`/api/health`、`/api/presets`、`/api/usage`、`/api/config` |

所有面向 Kindle 或脚本的响应默认发送 no-cache 头，减少旧状态被缓存的概率。

## 状态更新规则

`POST /api/status` 使用局部更新：
